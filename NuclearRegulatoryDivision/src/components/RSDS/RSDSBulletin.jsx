...
const RSDSBulletin = () => {
  ...
  const fetchBulletins = async () => {
    try {
      const response = await fetch("http://localhost:5175/RSDSBulletin");
      if (!response.ok) {
  ...
  const filteredBulletin = bulletin.filter(
    (item) =>
